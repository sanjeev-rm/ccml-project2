...
// Drive/walk to (tx,ty) with 2px steps; always animates while advancing
static void driveTo(DisplayObject& obj, int tx, int ty, int ms_per_step = 15) {
    tx = std::clamp(tx, 0, DisplayObject::WIDTH);
    ty = std::clamp(ty, 0, DisplayObject::HEIGHT);
    while (FarmLogic::_running && (obj.x != tx || obj.y != ty)) {
        int dx = (tx == obj.x) ? 0 : (tx > obj.x ? 2 : -2);
        int dy = (ty == obj.y) ? 0 : (ty > obj.y ? 2 : -2);
        obj.x = std::clamp(obj.x + dx, 0, DisplayObject::WIDTH);
        obj.y = std::clamp(obj.y + dy, 0, DisplayObject::HEIGHT);
        { std::lock_guard<std::mutex> lk(FarmLogic::_farmDisplayMutex); obj.updateFarm(); }
        std::this_thread::sleep_for(std::chrono::milliseconds(ms_per_step));
    }
}

// -------------------------------------------------------------
// Simple wandering animation helper (used by farmer, chickens, kids)
// -------------------------------------------------------------
static void wander(DisplayObject& obj, int duration_ms, int step = 6) {
    auto end = std::chrono::steady_clock::now() + std::chrono::milliseconds(duration_ms);
    while (FarmLogic::_running && std::chrono::steady_clock::now() < end) {
        int nx = std::clamp(obj.x + randStep(-step, step), 0, DisplayObject::WIDTH);
        int ny = std::clamp(obj.y + randStep(-step, step), 0, DisplayObject::HEIGHT);
        obj.x = nx;
        obj.y = ny;
        {
            std::lock_guard<std::mutex> g(FarmLogic::_farmDisplayMutex);
            obj.updateFarm();
        }
...
void FarmLogic::run() {
    ...
    auto chickenLogic = [&](DisplayObject& me){
        size_t idx = (me.id % nestMon.nests.size()); // start at different nests
        int laidHere = 0;
        while (_running) {
            auto target = nestMon.nests[idx];

            // walk to the nest (move visibly)
            driveTo(me, target->cx, target->cy, 15);

            // sit if there is space and seat free
            {
                std::unique_lock<std::mutex> lk(target->m);
                target->cv_space.wait(lk, [&]{ return !_running || (target->eggs < 3 && (target->sitter==-1 || target->sitter==me.id)); });
                if (!_running) break;
                target->sitter = me.id;
            }

            // lay 1–2 eggs, but never overfill
            ...
            for (int i=0;i<lay && _running;i++) {
                ...
                { std::lock_guard<std::mutex> glk(_farmDisplayMutex); stats.eggs_laid++; }
                std::this_thread::sleep_for(std::chrono::milliseconds(260));
                laidHere++;
            }

            // leave seat; farmer can collect
            {
            ...
            // visible “wander” so they don't camp
            wander(me, 600, 5);

            // force switch nests every ~3 eggs, with extra randomness for contention
        ...
        while (_running) {
            for (auto& n : nestMon.nests) {
                ...
                if (n->eggs > 0) {
                    ...
                    lk.unlock();
                    if (take>0) barnMon.addEggs(take);
                }
                // quick patrol move before next nest
                wander(me, 200, 4);
            }
        }
    };
    ...
    // --- Intersection for fairness (ticketed FIFO) ---
    struct TicketIntersection {
        std::mutex m;
        std::condition_variable cv;
        unsigned long nextTicket = 0, nowServing = 0;
        unsigned long takeTicket() { std::lock_guard<std::mutex> lk(m); return nextTicket++; }
        void enter(unsigned long my) { std::unique_lock<std::mutex> lk(m); cv.wait(lk, [&]{return my==nowServing;}); }
        void leave() { std::lock_guard<std::mutex> lk(m); nowServing++; cv.notify_all(); }
    } cross;

    // Pick a single crossing point so both trucks must coordinate to pass
    const int X_INTERSECT = 400;
    const int Y_EB = 420;  // egg/butter truck road y
    const int Y_FS = 380;  // flour/sugar truck road y

    auto truckEggButterLogic = [&](DisplayObject& t, int barnX, int bakeryX){
        // Path: start -> (barnX,Y_EB) -> INTERSECT -> (bakeryX,Y_EB) -> INTERSECT -> start ...
        while (_running) {
            // Go to barn
            driveTo(t, t.x, Y_EB);
            driveTo(t, barnX, Y_EB);

            // Load eggs + butter (eggs real, butter unlimited)
            // Wait here does NOT freeze animation because it occurs AFTER we arrived
            barnMon.takeEggs(3);
            std::this_thread::sleep_for(std::chrono::milliseconds(200)); // butter “load”

            // Cross intersection fairly
            {
                auto tk = cross.takeTicket(); cross.enter(tk);
                driveTo(t, X_INTERSECT, Y_EB);
                cross.leave();
            }
            // To bakery and unload
            driveTo(t, bakeryX, Y_EB);
            bakeryMon.unload(3,3,0,0);

            // Return through intersection
            {
                auto tk = cross.takeTicket(); cross.enter(tk);
                driveTo(t, X_INTERSECT, Y_EB);
                cross.leave();
            }
            // Back to a “depot” near start to loop (same road)
            // (You can pick any x to idle briefly)
            driveTo(t, std::min(barnX, bakeryX), Y_EB);
        }
    };

    auto truckFlourSugarLogic = [&](DisplayObject& t, int barnX, int bakeryX){
        while (_running) {
            // Go to barn
            driveTo(t, t.x, Y_FS);
            driveTo(t, barnX, Y_FS);
            ...
            {
                auto tk = cross.takeTicket(); cross.enter(tk);
                driveTo(t, X_INTERSECT, Y_FS);
                cross.leave();
            }
            // To bakery and unload
            driveTo(t, bakeryX, Y_FS);
            ...
            driveTo(t, std::max(barnX, bakeryX), Y_FS);
        }
    };

    // Launch the two trucks
    _workers.emplace_back(truckEggButterLogic, std::ref(truckEggButter), barnEggButter.x, bakery.x);
    _workers.emplace_back(truckFlourSugarLogic, std::ref(truckFlourSugar), barnFlourSugar.x, bakery.x);
    ...
    const int QUEUE_X = bakery.x - 30;
    const int QUEUE_Y = bakery.y + 50;
    const int COUNTER_X = bakery.x + 10;
    const int COUNTER_Y = bakery.y + 10;

    auto childLogic = [&](DisplayObject& kid){
        while (_running) {
            // move to queue
            driveTo(kid, QUEUE_X + randStep(-8,8), QUEUE_Y + randStep(-8,8), 20);

            // now try to go to counter (one at a time), buy k cakes
            int want = 1 + randStep(0,5); // 1..6
            {
                // get the counter
                std::unique_lock<std::mutex> lk(bakeryMon.m);
                bakeryMon.cv_counterFree.wait(lk, [&]{ return !_running || !bakeryMon.counterOccupied; });
                if (!_running) break;
                bakeryMon.counterOccupied = true;
                lk.unlock();
            }

            // walk to counter visibly
            driveTo(kid, COUNTER_X, COUNTER_Y, 18);

            // buy (waits if not enough cakes)
            bakeryMon.buyCakes(want);

            // leave counter
            {
                std::lock_guard<std::mutex> lk(bakeryMon.m);
                bakeryMon.counterOccupied = false;
                bakeryMon.cv_counterFree.notify_one();
            }

            // wander away a bit before re-queueing
            wander(kid, 800);
        }
    };
